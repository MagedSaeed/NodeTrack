...
export const TIME_PERIODS = {
  ...
  export const processTimeSeriesData = (data, selectedPeriod, selectedNodes) => {
    ...
    const aggregatedData = sortedTimestamps.map(timestamp => {
      ...
      Object.entries(nodeData).forEach(([nodeName, periodData]) => {
        ...
        if (values.used.length > 0) {
          // Using lodash's mean function for averaging
          // Assuming _ is imported in the component using this function
    ...
    // Reduce the number of visible points for better performance
    const maxPoints = 100;
