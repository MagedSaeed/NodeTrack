// CPU-specific utilities for time series visualization
...
// CPU-specific data processing
export const processTimeSeriesDataCPU = (data, selectedPeriod, selectedNodes, startDateString, endDateString) => {
  ...
  data.time_series.nodes_timeseries.forEach(node => {
    ...
    timeseries.forEach(point => {
      ...
      if (pointDate >= startDate && pointDate <= endDate && pointDate <= now) {
        ...
        nodeData[nodeName][periodKey].usage.push(point.usage_percent);
  ...
  const aggregatedData = sortedTimestamps.map(timestamp => {
    ...
    Object.entries(nodeData).forEach(([nodeName, periodData]) => {
      const values = periodData[timestamp] || { usage: [], active: [] };
      if (values.usage.length > 0) {
        const usageValue = values.usage.reduce((a, b) => a + b, 0) / values.usage.length; // Average
        const isActive = values.active.length > 0 ? values.active[values.active.length - 1] : true;

        dataPoint[nodeName] = usageValue;
        dataPoint[`${nodeName}_is_active`] = isActive;

        // Only add to totals if node is selected
        if (selectedNodes[nodeName]) {
          selectedUtilization += usageValue;
          selectedNodeCount++;
        }
      }
    });

    // Add average utilization for selected nodes
    dataPoint.total_utilization = selectedNodeCount > 0 ? selectedUtilization / selectedNodeCount : 0;
...
// CPU-specific detail point data creation
export const createDetailPointDataCPU = (pointData, selectedNodes, formatTimeLabel) => {
